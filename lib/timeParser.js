...
function parseTimeFromComments(comments = []) {
  ...
  for (const comment of comments) {
    ...
    // These are APPROVAL / CONFIRMATION phrases — they should only set the estimate
    // when there is no prior estimate (they confirm, not add).
    m = body.match(/(?:proceder\s+con\s+las?|aprobar|aprueba[n]?\s+las?|consumir\s+(?:en\s+el\s+bloque)?)\s+([\d.,]+)\s*hora/i);
    if (m) { const v = toHours(m[1] + ' horas'); if (v && estimated === null) estimated = v; }
    ...
    // --- EXECUTED time patterns (accumulate across comments) ---
    // "Tiempo ejecutado: 2h" / "Tiempo ejecutado 2h 30m"
    m = body.match(/tiempo\s+ejecutado\s*[:\-]?\s*([\d.,]+\s*h(?:\s*\d+\s*m)?|\d+(?:\.\d+)?\s*hora\w*)/i);
    if (m) { const v = toHours(m[1]); if (v) executed = (executed || 0) + v; }

    // "Horas ejecutadas: X"
    m = body.match(/hora\w*\s+ejecutad\w*\s*[:\-]?\s*([\d.,]+)/i);
    if (m) { const v = parseFloat(m[1]); if (v) executed = (executed || 0) + v; }
