...
/**
 * Analyze issue comments to classify deploy intent using Gemini AI.
 * Returns { isDeployAuthorized, hasPrdPr, isDeployValidated, deployDate, reason }
 *
 * @param {string} issueKey — Jira issue key (e.g. PY06809-63)
 * @param {Array<{author: string, body: string, created: string}>} comments — simplified comment list
 */
async function classifyDeployIntent(issueKey, comments) {
  if (!model) {
    if (!initGemini()) {
      // Fallback — cannot classify without AI
      return { isDeployAuthorized: false, hasPrdPr: false, isDeployValidated: false, deployDate: null, reason: 'AI no disponible' };
    }
  }

  // Build a compact comment summary for the prompt (limit tokens)
  const commentSummary = comments
    .slice(-8) // last 8 comments
    .map((c, i) => `[${i + 1}] ${c.author} (${c.created?.slice(0, 10) || '?'}): ${c.body.slice(0, 400)}`)
    .join('\n\n');

  const prompt = `Analiza los comentarios de la tarea ${issueKey} y responde SOLO con un JSON (sin markdown, sin explicación).

CONTEXTO: En este proyecto, las tareas pasan por este flujo de producción:
1. Se desarrolla y se genera un PR a "develop" (DEV)
2. Se solicita/genera un PR a "master/main" (producción/PRD)
3. El cliente autoriza el paso a producción
4. Se despliega y el cliente confirma que fue exitoso

PREGUNTA: Basándote en los comentarios, clasifica el estado de deploy:

RESPONDE EXACTAMENTE con este formato JSON:
{
  "isDeployAuthorized": boolean,  // ¿El CLIENTE autorizó explícitamente el paso a producción? (no solo pidió que se haga el PR)
  "hasPrdPr": boolean,           // ¿Se generó/entregó un PR apuntando a master/main/producción?
  "isDeployValidated": boolean,  // ¿El cliente confirmó que el deploy fue exitoso?
  "deployDate": "string|null",   // Fecha programada para despliegue si se menciona (ISO 8601)
  "reason": "string"             // Explicación breve (1 línea) de tu clasificación
}

REGLAS:
- "Por favor genera el PR" o "ayuda entregando el PR" = el cliente PIDE que se genere el PR, NO es autorización de deploy
- Autorización de deploy = frases como "autorizado para producción", "aprobado para subir a prod", "se puede pasar a producción"
- hasPrdPr = true solo si hay evidencia de un PR ya creado/entregado con URL o mención explícita de "PR a master/producción generado"
- isDeployValidated = true si el cliente dice "paso exitoso", "ya está en producción", "deploy exitoso"
...
${commentSummary}`;
  ...
  try {
    ...
    return {
      isDeployAuthorized: !!parsed.isDeployAuthorized,
      hasPrdPr: !!parsed.hasPrdPr,
      isDeployValidated: !!parsed.isDeployValidated,
      deployDate: parsed.deployDate || null,
      reason: parsed.reason || '',
    };
  } catch (err) {
    console.error(`⚠️  classifyDeployIntent(${issueKey}) error:`, err.message);
    return { isDeployAuthorized: false, hasPrdPr: false, isDeployValidated: false, deployDate: null, reason: `Error AI: ${err.message}` };
  }
}

module.exports = { generateActivityReport, generateBlockReport, formatText, reformatCommentForJira, classifyDeployIntent, initGemini };
