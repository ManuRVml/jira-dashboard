...
// Prefers DeepSeek (DEEPSEEK_API_KEY) → falls back to Gemini (GEMINI_API_KEY)

let _generateText = null;

function initAI() {
  const deepseekKey = process.env.DEEPSEEK_API_KEY;
  if (deepseekKey) {
    console.log('🤖 AI provider: DeepSeek (deepseek-chat)');
    _generateText = async (prompt) => {
  ...
  console.warn('⚠️  No AI provider configured. Set DEEPSEEK_API_KEY or GEMINI_API_KEY in .env');
  return false;
}

async function generateText(prompt) {
  if (!_generateText) {
    if (!initAI()) throw new Error('No AI provider configurado. Agrega DEEPSEEK_API_KEY o GEMINI_API_KEY en .env');
  }
...
${rawText}`;
  ...
  const fullText = (await generateText(prompt)).trim();
...
{
  "isDeployAuthorized": boolean,
  "hasPrdPr": boolean,
  "isDeployValidated": boolean,
  "deployDate": "string|null",
  "reason": "string"
}

REGLAS:
- "Por favor genera el PR" = el cliente PIDE que se genere el PR, NO es autorización de deploy
- Autorización de deploy = frases como "autorizado para producción", "aprobado para subir a prod"
- hasPrdPr = true solo si hay evidencia de un PR ya creado/entregado con URL o mención explícita
- isDeployValidated = true si el cliente dice "paso exitoso", "ya está en producción"
...
module.exports = { generateActivityReport, generateBlockReport, formatText, reformatCommentForJira, classifyDeployIntent, initAI };
